...
<head>
    ...
    <script  type="text/javascript">
        function deleteleaveifo(id) {
            //删除时安全提示
            if (confirm("是否确认删除？")){
                location.href="deleteleaveifo?id="+id
            }
        }
    </script>
</head>
<body>
        ...
        <form action="findByCondition" method="post"  autocomplete="off">
            ...
            <input type="text" class="inputs" id="startdate" name="startdate" value="${ConditionStartDate}" placeholder="&nbsp;点击选择日期" style="padding-left: 5px;width:100px;height: 30px;">
            <a style="margin-left:5px;font-size: 15px;color: #009688;margin-left: -5px;">——</a>
            <input type="text" class="inputs" id="enddate" name="enddate" value="${ConditionEndDate}" placeholder="&nbsp;点击选择日期" style="padding-left: 5px;width:100px;height: 30px;margin-left: -5px;">
        ...
        <table border="1" class="hovertable">
            ...
            <c:forEach var="leaveifo" items="${pageInfo.list}" varStatus="s">
                <%-- 用EL访问作用域变量的成员 --%>
                <tr onmouseover="this.style.backgroundColor='#ffff66';" onmouseout="this.style.backgroundColor='#d4e3e5';">
                    <td>${leaveifo.id}</td>
                    <td>${leaveifo.eno}</td>
                    <td>${leaveifo.ename}</td>
                    <td>${leaveifo.startdate} 至 ${leaveifo.enddate}</td>
                    <td>${leaveifo.type}</td>
                    <td>${leaveifo.agreer}</td>
                    <td><a class="button1" href="findById?id=${leaveifo.id}">修改</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                        <a class="button2" href="javascript:deleteleaveifo(&quot;${leaveifo.id}&quot;)">删除</a></td>
                </tr>
            </c:forEach>
        </table><br><br>
